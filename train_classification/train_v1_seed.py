...
def calculate_rc_consistency(model, test_path, device, model_type="hyenadna", num_samples=1000, seq_len=1024, seed=42):
    model.eval()
    try:
    ...
    with torch.no_grad():
        for i in range(0, len(sequences), batch_size):
            ...
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                logits_fwd = model(ids_fwd, attention_mask=mask_fwd)
                logits_rc = model(ids_rc, attention_mask=mask_rc)
...
def validate(model, test_loader, device, criterion_cls):
    model.eval()
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc=" Validating", leave=False):
            ...
            val_loss += loss.item()
            val_preds.extend(torch.argmax(logits.float(), 1).cpu().numpy())
...
def train_single_seed(current_seed):
    ...
    experiment_name = f"step1_{CONFIG['arch_type']}_{CONFIG['model_type']}_seed={current_seed}_aug_shift={CONFIG['aug_shift']}_rc={CONFIG['aug_rc']}"
    ...
    with open(log_file, "w") as f:
        csv.writer(f).writerow(
            ["Epoch", "Global_Step", "Train_Loss", "Val_Loss", "Val_MCC", "Val_Acc", "Train_Acc", "LR", "RC_Pearson"])

    best_mcc = -1.0
    global_step = 0

    print(f" Start Step 1 Training Seed {current_seed}...")
    for epoch in range(CONFIG["epochs"]):
        ...
        for step, batch in enumerate(pbar):
            ...
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                logits = model(input_ids, attention_mask=mask)
                loss = criterion_cls(logits.float(), labels) / CONFIG["grad_accumulation_steps"]
                stats["correct"] += (torch.argmax(logits, dim=1) == labels).sum().item()
            ...
            if (step + 1) % CONFIG["grad_accumulation_steps"] == 0:
                ...
                pbar.set_postfix({"Loss": f"{current_train_loss_val:.3f}", "LR": f"{current_lr:.1e}"})

                if global_step % CONFIG["val_check_interval"] == 0:
                    val_mcc, val_acc, val_loss = validate(model, test_loader, device, criterion_cls)
                    rc_pearson = calculate_rc_consistency(model, CONFIG["test_path"], device,
                                                          model_type=CONFIG["model_type"], num_samples=500,
                                                          seed=current_seed)

                    with open(log_file, "a") as f:
                        csv.writer(f).writerow([
                            epoch + 1, global_step, f"{current_train_loss_val:.4f}", f"{val_loss:.4f}",
                            f"{val_mcc:.4f}", f"{val_acc:.4f}", f"{stats['correct'] / stats['n']:.4f}",
                            f"{current_lr:.1e}", f"{rc_pearson:.4f}"
                        ])

                    if val_mcc > best_mcc:
                        best_mcc = val_mcc
                        torch.save(model.state_dict(), os.path.join(save_dir, "best_model.pth"))
                        pbar.set_postfix_str(f" Best MCC: {best_mcc:.4f}")


        del input_ids, mask, labels, logits, loss
