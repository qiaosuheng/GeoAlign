...
CONFIG = {
    ...
    # "foundation_ckpt": os.path.join(PROJECT_ROOT, "checkpoints", f"tapt_hyenadna_r=16_fixed", "tapt_hyenadna_epoch_2.pth"),
...
def calculate_rc_consistency_siamese_reg(model, val_df, device, model_type="hyenadna",
                                         num_samples=1000, seq_len=1024):
    """
     RC  (Siamese) forward_one_branch 
     Pearson  target 
    """
    model.eval()
    try:
    ...
    with torch.no_grad():
        for i in range(0, len(sequences), batch_size):
            ...
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                ...
                out_fwd = logits_fwd.float().cpu().numpy()
                out_rc = logits_rc.float().cpu().numpy()
...
def validate_siamese_reg(model, test_loader, device, criterion):
    """
     ()
    """
    model.eval()
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc=" Validating (Full)", leave=True):
            input_ids = batch["input_ids"].to(device)
            mask = batch["attention_mask"].to(device)
            labels = batch["labels"].to(device)

            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                preds, _ = model.forward_one_branch(input_ids, mask)
...
def train(current_seed, val_df_in_memory):
    ...
    experiment_name = f"reg_ours_{CONFIG['arch_type']}_{CONFIG['model_type']}_lambda={CONFIG['lambda_align']}_seed={current_seed}_shift={CONFIG['use_shift']}_rc={CONFIG['use_rc']}"
    ...
    print("\n" + "=" * 60)
    print(f" [SEED {current_seed}] Regression Training (Siamese)")
    ...
    train_ds = build_dataset(
        ...
        use_rc=CONFIG["use_rc"], use_shift=CONFIG["use_shift"], task_type="regression", target_cols=TARGET_COLS
    )
    ...
    header = ["Epoch", "Global_Step", "Train_Loss", "Align_Loss", "Val_Loss", "Val_Pearson_Avg"]
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        for step, batch in enumerate(pbar):
            ...
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                preds_anchor, preds_pair, feat_anchor, feat_pair = model(
                    input_ids, input_ids_pair, mask, mask_pair, return_feats=True
                )
                loss_reg = (criterion_reg(preds_anchor.float(), labels) + criterion_reg(preds_pair.float(), labels)) / 2
                loss_align = criterion_align(feat_anchor.float(), feat_pair.float())
                loss = (loss_reg + CONFIG["lambda_align"] * loss_align) / CONFIG["grad_accumulation_steps"]
            ...
            if (step + 1) % CONFIG["grad_accumulation_steps"] == 0:
                ...
                pbar.set_postfix({"Loss": f"{current_train_loss_val:.4f}", "Align": f"{current_align_loss_val:.4f}"})

                if global_step % CONFIG["val_check_interval"] == 0:
                    metrics, avg_pearson, val_loss = validate_siamese_reg(model, test_loader, device, criterion_reg)

                    rc_pearson = calculate_rc_consistency_siamese_reg(
                        model, val_df_in_memory, device,
                        model_type=CONFIG["model_type"], num_samples=1000
                    )

                    row = [epoch + 1, global_step, f"{current_train_loss_val:.4f}",
                           f"{current_align_loss_val:.4f}", f"{val_loss:.4f}", f"{avg_pearson:.4f}"]
                    for col in TARGET_COLS:
                        m = metrics[col]
                    ...
                    print(
                        f"\n    Step {global_step} | Val Loss: {val_loss:.4f} | {metric_str} | Align: {current_align_loss_val:.4f} | RC: {rc_pearson:.4f}")

                    if avg_pearson > best_pearson:
                        best_pearson = avg_pearson
                        torch.save(model.state_dict(), os.path.join(CONFIG["save_dir"], "best_model.pth"))
                        pbar.set_postfix_str(f" New Best Pearson: {best_pearson:.4f}")

        # --- Epoch End ---
        rc_pearson_full = calculate_rc_consistency_siamese_reg(
            model, val_df_in_memory, device,
            model_type=CONFIG["model_type"], num_samples=2000
        )

        with open(log_file, "a") as f:
            row = [epoch + 1, "End", f"{current_train_loss_val:.4f}",
                   f"{current_align_loss_val:.4f}", "0.0", "0.0"]
